...
cursor-agent -p --force "Read the code changes in the diff file at $DIFF_FILE. Compare them strictly against the JIRA Acceptance Criteria: \"$JIRA_AC\" - IF PROVIDED. Determine if the code satisfies the requirements. Give a bullet point breakdown of what passes and what fails. Write this to $OUTPUT_DIR/00_ac_check.md"

echo "✅ Main Agent finished. Spawning Sub-Agent Swarm..."

# 3. Sub-Agents (Parallel) - The "&" puts them in background
# React Best Practices
cursor-agent -p --force "If there are React files in the diff, review the Added Code in $DIFF_FILE solely for React Best Practices (e.g. Hooks rules, composition, re-renders, state management). Write findings in bullet points and with short sentences to $OUTPUT_DIR/01_react.md" &
echo "✅ Spawned React..."
sleep 0.5


# TypeScript Best Practices
cursor-agent -p --force "Review the Added Code in the diff file $DIFF_FILE solely for TypeScript Best Practices (e.g Strict typing, avoidance of any, interface segregation). Write findings in bullet points and with short sentences to $OUTPUT_DIR/02_ts.md" &
echo "✅ Spawned TypeScript..."
sleep 0.5

# Unit Test Best Practices
cursor-agent -p --force "If there are Unit Test files in the diff, review the Added Code in $DIFF_FILE solely for Unit Testing Best Practices (Jest, React Testing Library, Kent C. Dodds philosophy for testing, coverage, mocking logic, assertions). Write findings in bullet points and with short sentences to $OUTPUT_DIR/03_unit_tests.md" &
echo "✅ Spawned Unit Tests..."
sleep 0.5

# Cypress Component Test Best Practices
cursor-agent -p --force "If there are Cypress files in the diff, review $DIFF_FILE solely for Cypress Component Testing Best Practices (Selectors, interception, mounting). Write findings in bullet points and with short sentences to $OUTPUT_DIR/04_cypress.md" &
echo "✅ Spawned Cypress..."
sleep 0.5

# Uncle Bob's Clean Code
cursor-agent -p --force "Review the Added Code in $DIFF_FILE solely for 'Clean Code' principles by Uncle Bob (e.g Function size, naming conventions, structure and abstraction, clarity and intent, dependencies and coupling, SOLID principles, comments). Write findings in bullet points and with short sentences to $OUTPUT_DIR/05_clean_code.md" &
echo "✅ Spawned Clean Code..."
sleep 0.5

# Accessibility Best Practices - A11y
cursor-agent -p --force "Review the Added Code in $DIFF_FILE solely for 'a11y' principles (e.g. accessibility attributes, data-gtm-id, aria-label, role, etc.). Write findings in bullet points and with short sentences to $OUTPUT_DIR/06_a11y.md" &
...
cursor-agent -p --force "Review the Added Code in $DIFF_FILE solely for 'performance' principles (e.g. lazy loading, code splitting, image optimization, debouncing, throttling, etc.). Write findings in bullet points and with short sentences to $OUTPUT_DIR/08_performance.md" &
