...
cursor-agent -p --force "Read the code changes in $DIFF_FILE. Compare them strictly against these JIRA Acceptance Criteria: \"$JIRA_AC\". Determine if the code satisfies the requirements. detailed breakdown of what passes and what fails. Write this to $OUTPUT_DIR/00_ac_check.md"
...
cursor-agent -p --force "Review the Added Code in the diff file $DIFF_FILE solely for TypeScript Best Practices (e.g Strict typing, avoidance of any, interface segregation). Write findings to $OUTPUT_DIR/02_ts.md" &
